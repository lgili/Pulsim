...
Same fixture as fig52_etree (buck-like 8x8).
"""
...
def render(output_dir: Path) -> None:
    ...
    # Equal-size square panels via gridspec_kw={"aspect": "equal"}
    # implicit via set_aspect inside the loop, plus uniform figsize
    fig, axes = plt.subplots(1, 3, figsize=(8.0, 3.0))

    panels = [
        (a_pat,   f"(a) Original $|A|$\nnnz = {nnz_A}"),
        (nat_pat, f"(b) $L+U$ natural\nnnz = {nnz_nat}"
                   f"  ({nnz_nat - nnz_A:+d})"),
        (rcm_pat, f"(c) $L+U$ RCM\nnnz = {nnz_rcm}"
                   f"  ({nnz_rcm - nnz_A:+d})"),
    ]
    ...
    for ax, (mat, title) in zip(axes, panels):
        ax.spy(sp.csr_matrix(mat), markersize=8, color="#1b1b1b")
