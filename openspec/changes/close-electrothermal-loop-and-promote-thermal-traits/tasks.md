...
- [ ] 1.1.1 In `core/src/v1/transient_services.cpp`, locate
...
- [ ] 1.1.2 The walker MUST be in the SAME order as the existing
      `refresh_scales` walker so `T_i[device_index]` aligns to the
      correct device. Reuse `circuit_.devices()` iteration index.
- [ ] 1.1.3 Add a `set_T_j_init` declaration check via SFINAE / concept
...
- [ ] 1.2.1 Identify every Catch2 test that uses `R_th_ja > 0` on a
...
- [ ] 1.2.2 Update assertion tolerances in
...
- [ ] 1.3.1 New `core/tests/test_electrothermal_loop_closure.cpp`.
...
### 2.1 IdealDiode
- [ ] 2.1.1 In `core/include/pulsim/v1/components/ideal_diode.hpp`
      (search for `template<> struct device_traits<IdealDiode>`),
      flip `has_thermal_model` from `false` to `true`.
- [ ] 2.1.2 Verify `set_T_j_init` exists on IdealDiode. If missing,
...
### 2.2 Resistor
- [ ] 2.2.1 Flip trait in `core/include/pulsim/v1/components/resistor.hpp`.
- [ ] 2.2.2 Add `set_T_j_init` if missing.
- [ ] 2.2.3 Confirm `Resistor::R_at_Tj()` honors the new T_j(t) (it
      uses the temperature coefficient `TCR`).

### 2.3 Inductor
- [ ] 2.3.1 Flip trait in `core/include/pulsim/v1/components/inductor.hpp`.
- [ ] 2.3.2 Add `set_T_j_init` if missing.
- [ ] 2.3.3 Confirm `Inductor::DCR_at_Tj()` honors the new T_j(t).

### 2.4 Capacitor
- [ ] 2.4.1 Flip trait in `core/include/pulsim/v1/components/capacitor.hpp`.
- [ ] 2.4.2 Add `set_T_j_init` if missing.
- [ ] 2.4.3 Confirm `Capacitor::ESR_at_Tj()` honors the new T_j(t).

### 2.5 Per-device Circuit accessors
- [ ] 2.5.1 In `core/include/pulsim/v1/runtime_circuit.hpp`, ensure the
...
- [ ] 3.1.2 For each added type, the YAML block accepts:
      ...
      schema.
- [ ] 3.1.3 Add a small YAML regression test: a buck circuit defined
...
- [ ] 4.1.1 In `test_electrothermal_loop_closure.cpp`, add an assertion
...
- [ ] 4.1.2 Document the contract in
...
- [ ] 5.1 Update `docs/thermal-and-loss-models-audit.md` § "Top 10
      ...
      this change.
- [ ] 5.2 Update `docs/electrothermal-workflow.md` with the closed-loop
...
- [ ] 6.1 Full Catch2 suite must stay green:
      `pulsim_tests` + `pulsim_simulation_tests` zero new failures.
- [ ] 6.2 The new `[electrothermal_closure]` tag must have ≥ 6 cases
