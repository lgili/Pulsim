...
- **WHEN** `spicelab run circuit.json --tstop 0.01 --dt 1e-6` is executed
- **THEN** command-line options override model defaults
- **AND** simulation runs with specified parameters

#### Scenario: Output file
- **WHEN** `spicelab run circuit.json -o result.csv` is executed
...
- **WHEN** `spicelab validate circuit.json` is executed
...
- **WHEN** `spicelab convert circuit.json -o circuit.yaml` is executed
- **THEN** the model is converted between formats
- **AND** semantic content is preserved

#### Scenario: Convert results format
- **WHEN** `spicelab convert result.csv -o result.hdf5` is executed
...
- **WHEN** `spicelab sweep circuit.json --param R1=100:1000:10` is executed
...
- **WHEN** `spicelab serve` is executed
