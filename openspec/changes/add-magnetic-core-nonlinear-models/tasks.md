...
- [ ] 1.1 Define canonical YAML contract for nonlinear magnetic-core configuration (`component.magnetic_core`), including model family, parameter sets, initialization, and loss policy fields.
- [ ] 1.2 Add strict parser validation and deterministic diagnostics for missing fields, inconsistent dimensions, invalid ranges, and unsupported model-family/type combinations.
- [ ] 1.3 Define canonical kernel-side option/result structures for nonlinear magnetic-core state and loss telemetry.

## 2. Kernel Runtime Implementation
- [ ] 2.1 Implement nonlinear saturation behavior for supported magnetic components with deterministic state semantics.
- [ ] 2.2 Implement hysteresis behavior with explicit memory-state initialization and update ordering.
- [ ] 2.3 Implement frequency-dependent core-loss evaluation and coupling into loss summaries.
- [ ] 2.4 Integrate magnetic-core losses with electrothermal pipeline when enabled, without silent fallback.
- [ ] 2.5 Ensure fail-fast typed diagnostics for unsupported magnetic models or invalid runtime conditions.

## 3. Result Surface and Telemetry
- [ ] 3.1 Expose canonical magnetic channels and metadata (state/loss quantities, units, source component).
...
- [ ] 4.3 Propagate structured parser/runtime diagnostics with stable reason codes and field context.
...
- [ ] 5.3 Add canonical fixture circuits for frequency-dependent core-loss trend validation.
- [ ] 5.4 Add repeat-run determinism tests for magnetic-core channels and summaries.
- [ ] 5.5 Add KPI gates for fidelity, determinism, runtime, and allocation stability.

## 6. Documentation and UX Boundary
- [ ] 6.1 Document backend nonlinear magnetic-core contract (model families, parameters, outputs, diagnostics, limits).
- [ ] 6.2 Document frontend responsibilities and non-responsibilities for magnetic-core setup and visualization.
- [ ] 6.3 Publish YAML + Python examples and at least one tutorial notebook.
