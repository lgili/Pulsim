#!/usr/bin/env python3
"""Maia PFC FR — full validation sweep across all 3 operating points.

For each OP in (OP_2.2, OP_2.3, OP_2.4) this driver:

  1. Builds the front-end + inverter Pulsim sims
     (``maia_pfc_compressor_pulsim_validation.simulate_maia``).
...
from maia_pfc_compressor_pulsim_validation import (  # noqa: E402
    MaiaSimResult, make_sim_params, simulate_maia,
)
...
def extract_kpis(result: MaiaSimResult, losses: LossBreakdown,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Maia PFC FR — Pulsim validation sweep")
