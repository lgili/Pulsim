"""Maia PFC FR — per-device loss + thermal calculations.
...
# "Design Margins Maia FR" sheet (datasheet snapshots).
THERMAL = {
...
# ---------------------------------------------------------------------------
# Top-level loss reconstruction from a MaiaSimResult
# ---------------------------------------------------------------------------
...
def compute_losses(maia_result, *, settle_fraction: float = 0.5,
                   ) -> LossBreakdown:
    """Build a ``LossBreakdown`` from a ``MaiaSimResult``.
    ...
    """
    sp = maia_result.sim_params
    op = sp.op
    fe = maia_result.frontend
    inv = maia_result.inverter
