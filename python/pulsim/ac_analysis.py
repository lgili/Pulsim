...
# ``simulate`` and ``CircuitBuilder`` live on the top-level ``pulsim``
# package, but importing them at module scope here would deadlock —
# ``pulsim/__init__.py`` imports this submodule while still booting.
# Resolution is deferred to call time inside ``run_ac_sweep`` via
# ``import pulsim as p``.
...
def run_ac_sweep(
    ...
    *,
    ...
    # time (pulsim/__init__.py imports this submodule while still
    # booting; ``simulate`` isn't defined until the bottom of that
    # init).
    import pulsim as p
