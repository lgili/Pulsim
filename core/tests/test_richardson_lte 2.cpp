...
TEST_CASE("SolutionHistory - Basic operations", "[richardson][history]") {
    ...
    SECTION("Sufficient history check") {
        ...
        REQUIRE_FALSE(history.has_sufficient_history(2));  // Need 3 for order 2

        history.push(x, 1e-6, 1e-6);
        REQUIRE_FALSE(history.has_sufficient_history(2));  // Still need 3

        history.push(x, 2e-6, 1e-6);
        REQUIRE(history.has_sufficient_history(2));  // Now have 3
    }
}
