...
public:
    ...
    [[nodiscard]] DCAnalysisResult solve(
        ...
        // Configure Newton solver. Enable auto-damping by default —
        // the smooth MOSFET / IGBT / diode landscape that DC OP has
        // to walk has sigmoid tails ≈ 0 at x=0, then jumps to high
        // gradient once Vov > 0; a vanilla Newton step then
        // overshoots and oscillates between cutoff and triode for
        // certain (V_GATE, vth, kp) combinations. Step limiting and
        // trust region stay OFF here so we don't restrict legitimate
        // large-step circuits like boost DC OP where node voltages
        // can swing by tens of volts in a single iteration.
