...
public:
    ...
    /// total gap lg [m], initial μ_r, and B_sat [T] where μ_r has
    /// halved (p sets the knee sharpness). The λ(i) table is
    /// generated by sweeping the flux and evaluating Ampère's law —
    ...
    CircuitBuilder& add_gapped_core_inductor(
        ...
        Real p_knee = Real{4}, Size knots = 96) {
        models::GappedCore::Params c;
        c.N = N; c.Ae = Ae; c.le = le; c.lg = lg;
        c.mu_r0 = mu_r0; c.B_sat = B_sat; c.p = p_knee; c.knots = knots;
        auto table = std::make_shared<const models::FluxTable>(
        ...
        p.L_0 = table->L_0();
        p.L_residual = table->L_residual();
        p.I_sat = models::GappedCore::current_at_flux(c, N * Ae * B_sat);
        p.table = std::move(table);
        return add_saturable_inductor_params_(name, n_pos, n_neg, p);
    }
