...
## The buck circuit (`examples/v2/buck.yaml`)
...
```python
$ python examples/v2/scripts/run_buck.py

Loading examples/v2/buck.yaml ...
  num_branches = 7
  dt = 1e-07 s
  t_end = 0.005 s
  cache built ✓
...
- **Boost converter**: swap to `examples/v2/boost.yaml`
  (V1 add-on). The PWM bit-0 still controls the MOSFET;
  topology changes via the YAML.
- **Flyback** (`examples/v2/flyback.yaml`, already
