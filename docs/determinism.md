...
This guide covers how to ensure reproducible simulation results in PulsimCore v2.
...
```python
init_cfg = v2.InitializationConfig()
...
dc_cfg.strategy = v2.DCStrategy.GminStepping  # Deterministic
...
msg = v2.solver_status_to_string(status)
print(f"Status: {msg}")

if status == v2.SolverStatus.MaxIterationsReached:
    print("Increase max_iterations or improve initial guess")
elif status == v2.SolverStatus.SingularMatrix:
    print("Check circuit topology for floating nodes")
elif status == v2.SolverStatus.Diverging:
...
result = v2.compare_waveforms("test", sim_data, ana_data)
...
# 3. DC convergence
dc_cfg = v2.DCConvergenceConfig()
dc_cfg.strategy = v2.DCStrategy.GminStepping  # Not Auto
dc_cfg.enable_random_restart = False

# 4. Newton solver
newton_opts = v2.NewtonOptions()
newton_opts.track_history = True

# 5. Timestep (optional: fixed for maximum reproducibility)
ts_cfg = v2.TimestepConfig()
ts_cfg.dt_min = 1e-6
ts_cfg.dt_max = 1e-6

# 6. BDF order (optional: fixed)
bdf_cfg = v2.BDFOrderConfig()
bdf_cfg.min_order = 2
bdf_cfg.max_order = 2
...
5. **Compare platform details**:
   ```python
   print(f"SIMD level: {v2.detect_simd_level()}")
   print(f"Vector width: {v2.simd_vector_width()}")
   ```
