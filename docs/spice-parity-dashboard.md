...
| **9 — Phase-9 investigation (PWM not the bug)** | Initial Phase-9 hypothesis was "PWM duty > 0.5 doesn't toggle" but **the PWM source is correct in isolation** (verified at duty=0.64, NMOS in same buck topology toggles fine). The actual root cause in `buck_pmos` is **Shichman-Hodges + adaptive timestep**: at startup the high-side PMOS lands in deep saturation with id ≈ 44 A (Vov=21 V·kp=0.2 → unphysical for an 8 Ω load), Pulsim's adaptive integrator rejects 963 / ~1000 timesteps over a 30 µs trace, and the rejected samples carry the stale V(gate) forward. Phase 10 is now: improve adaptive-timestep behaviour during discontinuous source/device transitions, OR migrate `buck_pmos` to the catalog MOSFET tier (datasheet I-V curves) where R_DSon is realistic for power devices. |
