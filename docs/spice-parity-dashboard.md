...
| **11 — PMOS sign audit + saturation overdrive (deferred)** | Two related sub-problems: (i) audit the PMOS chain-rule in the manual stamp against an analytical derivation independent of the AD template; (ii) decide a path for the saturation overdrive — either a built-in current-clamp (so `id_sat ≤ I_max`), or migrate `buck_pmos` to the catalog MOSFET tier (datasheet I-V, R_DSon-driven). |
